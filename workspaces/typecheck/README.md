...
It is a minimal TanStack Start route tree with a `/help` source subtree mounted at `/inventory/help` (plus an overlapping direct mount of `/help/guides` at `/settings/guides`, a nested mount of `/modal` from inside the help subtree at `/help/modal`, and a stock `.lazy` pair). Generated files are committed (`gitignore: false`), so a bare `tsc` run works without codegen. The interesting parts:
