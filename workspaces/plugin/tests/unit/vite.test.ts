...
describe("sharedRoutes vite plugin", () => {
   ...
   it("reads shared-routes.config.json from the root, like the CLI", async () => {
      const root = makeTmpDir();
      writeTree(root, {
   ...
   it("lets inline options override the file per key and logs the override", async () => {
      const root = makeFixture();
      const fileBanner = "// Generated by tsr-shared-routes (file)";
      const inlineBanner = "// Generated by tsr-shared-routes (inline)";
      writeTree(root, {
         "shared-routes.config.json": JSON.stringify({ banner: fileBanner }),
      });
      const plugin = sharedRoutes({ banner: inlineBanner });
      await callConfig(plugin, root);
      expect(readFile(path.join(root, "src/routes/inventory/help/route.tsx"))).toContain(
         inlineBanner,
      );

      const warn = vi.fn();
      const hook = plugin.configResolved as Handler<Plugin["configResolved"]>;
      hook.call(undefined as never, { logger: { warn } } as never);
      const warned = warn.mock.calls.map((call) => String(call[0])).join("\n");
      expect(warned).toContain('config "banner"');
      expect(warned).toContain(inlineBanner);
      expect(warned).toContain(fileBanner);
   });

   it("does not log same-value duplicates across the two config sources", async () => {
      const root = makeFixture();
      writeTree(root, {
         "shared-routes.config.json": JSON.stringify({ gitignore: false }),
      });
      const plugin = sharedRoutes({ gitignore: false });
      await callConfig(plugin, root);
      const warn = vi.fn();
      const hook = plugin.configResolved as Handler<Plugin["configResolved"]>;
      hook.call(undefined as never, { logger: { warn } } as never);
