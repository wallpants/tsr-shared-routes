      ...
      try {
         ...
         for (const nested of scan.nestedMounts) {
            ...
            if (scaffold && scaffoldEmptyMountFile(nested.mountFilePath, code)) {
               ...
               skippedMounts++;
               incomplete.push(
                  `mount file ${nested.mountFilePath} is waiting for its shared-dir path`,
               );
               continue;
            }
            ...
            if (classified.kind === "invalid") {
               ...
               skippedMounts++;
               warnings.push(
                  `skipping invalid mount file ${nested.mountFilePath} (run \`tsr-shared-routes generate\` for details)`,
               );
               continue;
            }
            attempt(nested.mountFilePath, () => {
               validateMountName(nested.mountFilePath, config, warnings);
               const resolved = resolveSharedDir(
                  nested.mountFilePath,
                  classified.sharedDirRelative,
               );
