...
export function runPipeline(
  ...
  for (const { file, routeIdLiteral } of desired) {
    ...
    const ids = helperMountIds.get(file.sharedFilePath) ?? [];
    ids.push(routeIdLiteral);
    helperMountIds.set(file.sharedFilePath, ids);
  }
  const desiredHelpers = [...helperMountIds.entries()]
    .map(([sharedFilePath, mountIds]) => ({
      helperPath: helperPathFor(sharedFilePath),
      sharedFilePath,
  ...
  for (const helper of desiredHelpers) {
    const existing = readIfExists(helper.helperPath);
    if (existing !== undefined && !isOwned(existing)) {
      throw new SharedRoutesError(
        "UNOWNED_TARGET_FILE",
        `refusing to overwrite ${rel(helper.helperPath)}\nThis file exists but was not generated by tanstack-shared-routes.\n→ Remove the file (the \`.gen\` sibling name is reserved for generated helpers).`,
      );
    }
  ...
  // Helpers hold no route-id literal for the stock generator to correct, so a
  // plain byte compare replaces the masked diff (mask is a no-op on them).
  for (const helper of desiredHelpers) {
    finalContent.set(helper.helperPath, helper.content);
    if (existingByPath.get(helper.helperPath) === helper.content) {
      unchanged++;
    } else {
      if (!check) atomicWrite(helper.helperPath, helper.content);
      written.push(rel(helper.helperPath));
    }
  }
